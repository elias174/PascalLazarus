...
uses
  ...
  ucmdbox, StdCtrls, Controls, Buttons, Menus, solver_eq, fpexprpars, math, integral_solver, main;
...
procedure bisection_method(var Result: TFPExpressionResult; Const Args: TExprParameterArray);
var
  solver_eq: SolverEq;
  a,b, ret: Double;
  eq: String;
begin
  ...
  solver_eq.solve_a:= a;
  solver_eq.solve_b:= b;
  solver_eq.ErrorAllowed:= 0.001;

  solver_eq.MethodType:=0;
  solver_eq.fx:= eq;
  ret := solver_eq.execute();
  WriteLn(ret);
  Result.resFloat := ret;
end;

procedure trapezoidal_method(var Result: TFPExpressionResult; Const Args: TExprParameterArray);
var
...
begin
 ...
 graph:= TfrmGraficadora.Create(Application);
 graph.cboFuncion.Text:=fx;
 graph.AreaA.Text:=FloatToStr(a);
 graph.AreaB.Text:=FloatToStr(b);
 graph.ediN.Text:=IntToStr(n);
 graph.chkEscogerN.Checked:=True;
 graph.Show;
 graph.btnGraficarClick( nil );
 graph.Button1Click(nil);
...
begin
 ...
 FParser.Identifiers.AddFunction('bisection', 'F', 'SFF', @bisection_method);
 FParser.Identifiers.AddFunction('trapezoidal', 'F', 'SFFI', @trapezoidal_method);
...
begin
 ...
 begin
  ...
  else if pos('bisection', Input)<>0 then
  begin
  ...
  else if pos('trapezoidal', Input)<>0 then
  begin
     FParser.Expression := Input;
     result_method:= FParser.Evaluate.ResFloat;
     CmdBox.TextColors(clGreen,clBlue);
